...
# Writes to canonical paper_trades table from agent_config.yaml
# Enhancements: BE stop, partial TP, hard stop cap
# =============================================================================
...
def _simulate_trade(entry_price, future_prices, future_times, stop_pct, tp_pct,
    ...
    for j in range(1, min(max_hold_bars, len(future_prices) - 1) + 1):
        ...
        if px <= stop_price:
            ...
            exit_time = future_times[j]
            exit_reason = "stop_loss_be" if breakeven_activated else "stop_loss"
            chosen_idx = j
            break
...
def run_paper_trade_agent(cfg: dict, context: dict) -> dict:
    ...
    max_stop_pct = ptcfg.get("max_stop_pct", 0.025)  # hard cap 2.5%
    be_trigger = ptcfg.get("breakeven_trigger", 0.5)  # 0.5x RR triggers BE
    ...
    try:
        ...
        while i < n:
            ...
            # Signal filter: high confidence + bullish label
            if pred_proba < band_min or pred_label != 1:
                i += 1
                continue

            entry_price = float(row["close"])
            vol = float(row.get("vol_20", 0.01))
        ...
        if not trades_df.empty:
            # Write to canonical table from config
            con.execute(f"""
                CREATE TABLE IF NOT EXISTS {tables['paper_trades']} (
                    run_id VARCHAR,
            ...
            con.execute(f"INSERT INTO {tables['paper_trades']} SELECT * FROM trades_df")
